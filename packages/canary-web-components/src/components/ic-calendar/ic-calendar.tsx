...
export class Calendar {
  ...
  valueChanged(
    ...
    if (this.isPresent(newValue)) {
      this.value = newValue;
      console.log({ valueInWatcher: this.value });
    }
    if (newValue !== oldValue && !this.isUpdating) {
  ...
  private setInitialState = () => {
    ...
    console.log("checking updated value", {
      value: this.value,
    ...
    if (
      ...
      if (this.isPresent(this.openAtDate)) {
        openAt = createDate(this.openAtDate, this.dateFormat);
      }

      if (this.isPresent(this.value)) {
        const parsedValue = createDate(
          this.value ?? new Date(),
          this.dateFormat
        );
        console.log({ parsedValue });
        openAt = parsedValue;
        const selectedDay = parsedValue?.getDay();
