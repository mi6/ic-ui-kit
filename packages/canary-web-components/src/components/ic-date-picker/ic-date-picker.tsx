...
export class DatePicker {
  ...
  private hasHandledChange = false;
   ...
   */
  @Event() icChange: EventEmitter<{ value: Date | null }>;
  ...
  localCalendarButtonClickHandler(): void {
  ...
  dateChangeHandler(ev: CustomEvent): void {
    if (this.hasHandledChange) return;
    this.hasHandledChange = true;
    const path = ev.composedPath();
    const sourceElement = path[0] as HTMLElement;
    ev.stopImmediatePropagation();

    if (sourceElement === this.el) return;
    ...
    if (sourceElement.tagName === "IC-CALENDAR") {
      ...
      this.updateDate(newValue, false);
      this.openAtDate = newValue;
    }

    setTimeout(() => {
      this.hasHandledChange = false;
    }, 0);
  }

  private updateDate(newValue: Date | null, emit = true) {
    if (!dateMatches(newValue, this.selectedDate)) {
      this.selectedDate = newValue;
      this.value = newValue;
      if (emit) {
        this.inputEl?.triggerIcChange(newValue);
