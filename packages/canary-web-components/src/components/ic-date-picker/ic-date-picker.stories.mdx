...
<Canvas withSource="none">
  <Story name="Max width" height="790px">
    {MaxWidth()}
  </Story>
</Canvas>
...
<Canvas withSource="none">
  <Story name="Date formats" height="750px">
    {DateFormats()}
  </Story>
</Canvas>
...
### IcSelectedDateChange event

The `IcSelectedDateChange` event is emitted by the date picker when the selected date changes.

<Canvas withSource="none">
  <Story name="IcSelectedDateChange event" height="540px">
    {IcSelectedDateChange()}
  </Story>
</Canvas>

#### IcSelectedDateChange event code example

```html
<div style="display: flex; flex-direction: column; gap: 50px;">
  <ic-date-picker id="ic-change-datepicker" />
  <span id="selected-date"> Selected date: </span>
</div>
<script>
  const displaySelectedDate = (ev) => {
    console.log("Selected date: " + ev.detail.value);
  };
  const datePicker = document.querySelector("#ic-change-datepicker");
