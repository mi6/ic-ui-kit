...
export class TableOfContents {
   ...
   * @param validHeading heading element to refer item information from
   * @param children
   * @returns TocItem ready to be rendered, with nested children if applicable
   */
  private createTocItem = (
    validHeading: RankedElement,
    children?: HTMLElement[]
  ): HTMLElement => {
    if (!validHeading.id) {
      const id = validHeading.innerHTML.replace(/\s+/g, "-").toLowerCase();
      validHeading.setAttribute("id", id);
    }

    const tocId = `toc_${validHeading.id}`;

    const tocItem = (
      <TableOfContentsItem
        id={tocId}
        className={TOC_ITEM_CLASS}
        variant={validHeading.tagName.toLowerCase()}
        label={validHeading.innerHTML}
        target={validHeading.id}
        truncate={this.truncate}
        selected={this.focusedElement}
        updateSelected={this.updateOnSelection}
        position={validHeading.trueIndex}
        size={this.size}
      />
    );

    const wrappedToc = this.truncate ? (
      <ic-tooltip target={tocId} label={validHeading.innerHTML}>
        {tocItem}
      </ic-tooltip>
    ) : (
      tocItem
  ...
  private generateHeadingTree(): RankedElement[] {
    ...
    if (headingList) {
      ...
      headingArray.reverse().forEach((heading, index) => {
        if (!heading.id) {
          const id = heading.innerHTML.replace(/\s+/g, "-").toLowerCase();
          heading.setAttribute("id", id);
        }
        if (heading.rank > currentRank) {
          for (let n = index; n < headingArray.length; n += 1) {
            if (headingArray[n] && headingArray[n].rank < heading.rank) {
              headingArray[n].treeChildren.unshift(heading);
              break;
            }
          }
        } else {
          currentRank = heading.rank;
          resultArray.push(heading);
        }
      });
    }
