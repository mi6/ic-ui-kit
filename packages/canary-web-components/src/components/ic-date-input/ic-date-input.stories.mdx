...
To hide the label and helper text, set the `hide-label` attribute to `true`. The required `label` will still be read out by screen readers.

<Canvas withSource="none">
  <Story name="Hide label" parameters={{ loki: { skip: true } }}>
    {html`<ic-date-input
      label="When would you like to collect your coffee?"
      hide-label="true"
    ></ic-date-input> `}
