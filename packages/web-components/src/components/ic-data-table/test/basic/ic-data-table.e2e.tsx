  ...
  describe("truncation & textWrap", () => {
    ...
    it("should truncate using ic-typography's see more/see less button when truncationPattern is set to `showHide`", async () => {
      ...
      const { maxLinesProp, hasButton } = await page.evaluate(() => {
        const typographyEl = document.querySelector(dataTable).shadowRoot.querySelector("#jobTitle-2") as HTMLIcTypographyElement;
        return { maxLinesProp: typographyEl.maxLines, hasButton: !!typographyEl.shadowRoot.querySelector("button") }
      });
