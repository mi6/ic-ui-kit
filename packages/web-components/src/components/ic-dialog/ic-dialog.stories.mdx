...
<Canvas>
  <Story name="Hidden close icon" parameters={{ loki: { skip: true } }}>
    {html`
      ...
      <ic-button variant="primary" onclick="showHiddenCloseIconDialog()"
        >Launch dialog with hidden close icon</ic-button
      >
      <ic-dialog
        id="hidden-close-icon-dialog"
        heading="This dialog does not have a close icon"
        label="Hidden Close Icon Dialog"
        hide-close-icon
      >
        <ic-typography>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
