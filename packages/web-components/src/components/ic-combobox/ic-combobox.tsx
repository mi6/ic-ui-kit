...
import {
  ...
  getMenuOptions,
  getSortedOptions,
  getValueFromActivedescendantIndex,
} from "../../utils/combobox-helpers";
...
export class Combobox {
  ...
  activedescendantIndexHandler(): void {
    ...
    this.ariaActivedescendant = getAriaActivedescendant(
      this.listboxId,
      this.el,
  ...
  showClearButtonValidator(newValue: boolean) {
    if (!newValue) {
      ...
      console.warn(
        "show-clear-button prop updated to true. The clear button is always shown on the searchable variant."
      );
    }
  }
  ...
  validateMultipleSearchable() {
    if (this.multiple && this.searchable) {
      console.warn(
        "Searchable functionality can only be used with the single-select variant. Defaulting to the single-select variant"
      );
  ...
  private dispatchComboboxAction = (
    action?: IcComboboxAction,
    ...
    switch (action) {
      ...
      case IcComboboxAction.OpenListbox:
        if (this.value)
          currIndex = this.flattenedFilteredOptions
            .map((option) => option.value)
            .indexOf(this.valueString);
  ...
  private getActionFromKeyboardEvent = (
    event: KeyboardEvent
  ): IcComboboxAction | undefined => {
    ...
    if (this.open) {
      switch (key) {
        ...
        case "ArrowUp":
          event.preventDefault();
          return altKey
            ? IcComboboxAction.SelectAndCloseListbox
            : IcComboboxAction.PrevOption;
        case "Home":
  ...
  private handleComboboxClick = () => {
    this.dispatchComboboxAction(
      this.open ? IcComboboxAction.CloseListbox : IcComboboxAction.OpenListbox
    );
  };
  ...
  private handleFilter = (searchString: string) => {
    const filteredParents = getFilteredMenuOptions(
    ...
    this.processedOptions.forEach((option) => {
      ...
      if (children) {
        const filteredChildren = getFilteredMenuOptions(
          children,
          this.includeDescriptionsInSearch,
          searchString,
          this.searchMatchPosition
  ...
  private getLabelFromValue = (): string => {
    return !!this.value
      ? this.flattenedFilteredOptions.find(
  ...
  private clearUserInput = () => {
    const inputEl = this.el.shadowRoot?.querySelector<HTMLInputElement>(
      `#${this.inputId}`
    );
  ...
  render() {
    ...
    renderHiddenInput(this.el, valueString, name || inputId, disabled);

    const isUserInput = (): boolean => {
      const inputString = this.el.shadowRoot?.querySelector<HTMLInputElement>(
        `#${inputId}`
      )?.value;
      return !!inputString;
    };

    const isClearable = !disabled && !!isUserInput();
        ...
        <div class="select-container">
          <input
            ...
            aria-activedescendant={
              loadingStatus === IcComboboxLoadingStatus.Loaded
                ? ariaActivedescendant
                : ""
            }
