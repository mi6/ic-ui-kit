...
export const getAriaActivedescendant = (
  listboxId: string,
  el: HTMLIcSelectNewElement | HTMLIcComboboxElement,
  activedescendantIndex: number | null
): string => {
  const value = getValueFromActivedescendantIndex(
    el,
    listboxId,
    activedescendantIndex
  );
  return value ? getOptionIdFromValue(listboxId, value) : "";
};

export const getValueFromActivedescendantIndex = (
  el: HTMLIcSelectNewElement | HTMLIcComboboxElement,
  listboxId: string,
  activedescendantIndex: number | null
) => {
  ...
  const selectAllBtn = el.shadowRoot?.querySelector<HTMLIcButtonElement>(
    `#${getOptionIdFromValue(listboxId, "select-all")}`
  );
...
export const getFirstSelectedOptionIndex = (
  ...
  return firstSelectedOption
    ? flattenedOptions.indexOf(firstSelectedOption)
    : 0;
};

export const getDeduplicatedOptions = (
...
export const getFlattenedOptions = (groupedOptions: IcMenuOption[]) => {
  ...
  if (groupedOptions.length > 0) {
    groupedOptions.map((option) => {
      if (option.children) {
        option.children.map((option) => flattenedOptions.push(option));
      } else {
...
export const getMenuOptions = (
  options: IcMenuOption[],
  loadingStatus: IcComboboxLoadingStatus,
