...
export const LoadingExample = () => {
  ...
  return <IcDataTable caption="Empty and Loading state" columns={COLS} loading={loading}></IcDataTable>
}

<Canvas withSource="none">
  <Story name="Empty and Loading state">
    <LoadingExample />
  </Story>
</Canvas>

#### Empty and Loading state code example
...
const DataTable = () => {
  ...
  setTimeout(() => setLoading(true), 5000);
  return <IcDataTable
    caption="Empty and Loading state"
    columns={columns}
    loading={loading}
  />
};
...
export const UpdatingExample = () => {
  const [updating, setUpdating] = useState(false);
  setTimeout(() => setUpdating(true), 5000);
  return <IcDataTable caption="Updating state" columns={COLS} data={DATA} updating={true}></IcDataTable>
}
...
const data = [
  {
    ...
    jobTitle: "Developer",
    address: "1 Main Street, Town, County, Postcode",
    actions: ReactDOMServer.renderToString(<IcButton variant='destructive' onClick={() => this.closest("tr").remove()}>Delete</IcButton>),
  },
  ...
];
