...
async function runLhciForRoute(route) {
  ...
  const routeReportDir = path.join(pkgDir, `lhci-reports/${route.name}`);
  ...
  const configPath = path.join(pkgDir, `.lighthouserc.${route.name}.json`);
...
async function main() {
  ...
  const reportsDir = path.join(pkgDir, "lhci-reports");
