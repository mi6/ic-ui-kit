...
<Canvas withSource="none">
  <Story name="Exclude columns from sort">
    <IcDataTable caption="Sort" columns={COLS_EXCLUDE_SORT} data={DATA} sortable />
  </Story>
</Canvas>
...
const DataTable = () => (
  ...
  >
    {data.map((_, index) => (
      <IcButton key={`actions-${index}`} slot={`actions-${index}`} onClick={() => console.log(`${index+1}`)}>
        {Index+1}
      </IcButton>
      ))
    }
...
<Canvas withSource="none">
  <Story name="Turn off selected row highlighting">
    <IcDataTable caption="Turn off selected row highlighting" columns={COLS} data={DATA} highlightSelectedRow={false} />
  </Story>
</Canvas>
...
<Canvas withSource="none">
  <Story name="Description and icons within cells">
    <IcDataTable
...
<Canvas withSource="none">
  <Story name="Description and icons within cells with long text">
    <IcDataTable
      caption="Basic Table"
      columns={COLS_WIDTH}
