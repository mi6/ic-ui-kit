...
To reset all row heights to the default, you can run the `resetRowHeights` method.
...
export const UpdatingData = () => {
  ...
  const [pagination, setPagination] = useState([{ label: '5', value: '5' }]);
...
setPagination([{ label: '10', value: '10' }, { label: '20', value: '20' },{ label: '50', value: '50' }])
}

const resetPagination = () => {
setPagination([{ label: '5', value: '5' }]);
}
...
<>
  ...
  <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
    ...
    <IcButton onClick={updatePagination}>Update page lengths</IcButton>
    <IcButton onClick={clearData}>Reset data</IcButton>
    <IcButton onClick={resetPagination}>Reset page lengths</IcButton>
  </div>
</>
); };
