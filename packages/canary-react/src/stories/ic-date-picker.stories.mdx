...
### IcSelectedDateChange event - calendar only mode

The `IcSelectedDateChange` event is emitted by the calendar in the date picker when the selected date changes.

export const DatePickerWithIcSelectedDateChange = () => {
  const [selectedDate, setSelectedDate] = useState();
  const [value, setValue] = useState();
  const dateChangedHandler = (event) => {
    setValue(event.detail.value);
...
<Canvas withSource="none">
  <Story name="IcSelectedDateChange event - calendar only mode" height="540px">
    <DatePickerWithIcSelectedDateChange />
  </Story>
</Canvas>

#### IcSelectedDateChange event - calendar only mode code example
...
export default DatePickerWithIcSelectedDateChange;
```
