...
describe("IcDataTables", () => {
  ...
  it("renders", () => {
    ...
    cy.compareSnapshot({
      name: "default",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.066),
    });
  ...
  it("should render with row headers", () => {
    ...
    cy.compareSnapshot({
      ...
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.068),
    });
    cy.checkA11y(undefined, CYPRESS_AXE_OPTIONS);
  ...
  it("should render embedded", () => {
    ...
    cy.compareSnapshot({
      name: "embedded",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.067),
    });
  });
  ...
  it("should sort data when the sort button is clicked", () => {
    ...
    cy.compareSnapshot({
      name: "sortable",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.063),
    });
  ...
  it("should render with row overrides", () => {
    ...
    cy.compareSnapshot({
      name: "row-overrides",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.069),
    });
  ...
  it("should render the updating state when the `updating` prop is passed through", () => {
    ...
    cy.compareSnapshot({
      name: "updating",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.061),
    });
  });
  ...
  it("should apply a specified row height to specific rows when variableRowHeight is set, and any not included should use the default globalRowHeight", () => {
    ...
    cy.compareSnapshot({
      ...
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.079),
  ...
  it("should slot a custom element into a cell", () => {
    ...
    cy.compareSnapshot({
      ...
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.073),
    });
  });
  ...
  it("should apply a background to the loading indicator if data is supplied to the table", () => {
    mount(
      <IcDataTable caption="Data tables" columns={COLS} data={DATA} loading />
    );
  ...
  it("should cancel the loading state when the loading prop is `true` and data has been set after 1 second from initial loading", () => {
    ...
    //Check if loading indicator has show-background class when data is set
  ...
  it("should wait 1 second before rendering the data when the `loading` prop is passed through and data is set before 1 second of loading", () => {
    ...
    cy.compareSnapshot({
      ...
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.03),
    });
  ...
  it('should stick the header when "stickyHeader" and height is set', () => {
    ...
    cy.compareSnapshot({
      name: "sticky-header",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.126),
      cypressScreenshotOptions: {
        capture: "viewport",
      },
    });
    ...
    cy.compareSnapshot({
      ...
      testThreshold: setThresholdBasedOnEnv(DEFAULT_THRESHOLD + 0.115),
      cypressSCreenshotOptions: {
        capture: "viewport",
      },
    });
