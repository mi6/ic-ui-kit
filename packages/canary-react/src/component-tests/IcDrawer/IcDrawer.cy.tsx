...
describe("IcDrawer", () => {
  ...
  it("should render at the top of the viewport", () => {
    mount(<IcDrawer heading="Menu" message="test" position="top" expanded />);
  ...
  it("should render in a dark theme", () => {
    mount(<IcDrawer heading="Menu" message="test" expanded theme="dark" />);

    cy.checkHydrated(DRAWER_SELECTOR);

    cy.checkA11yWithWait();
    cy.compareSnapshot({
      name: "/dark-theme-collapsed",
      testThreshold: setThresholdBasedOnEnv(DEFAULT_TEST_THRESHOLD),
    });
  ...
  it("should set the aria label of the chevron button to the value of chevronButtonAriaLabel when chevronButtonAriaLabel is not an empty string", () => {
    ...
    cy.compareSnapshot({
      ...
      testThreshold: setThresholdBasedOnEnv(DEFAULT_TEST_THRESHOLD + 0.131),
    });
  });
  ...
  it("should close the drawer when closeOnBackdropClick is set to true and the backdrop is clicked", () => {
    mount(
      <IcDrawer
        heading="Menu"
        message="test"
    ...
    cy.get(DRAWER_SELECTOR).should(NOT_HAVE_ATTR, "expanded");
  });
